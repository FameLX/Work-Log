...
let archived = JSON.parse(localStorage.getItem("wl2dev_archived") || "[]");
...
// ── Settings state ─────────────────────────────────────────────────────────
const BUILTIN_TYPES = ["Meeting", "Review", "Report", "Research", "Other"];
...
function buildGTaskTitle(entry) {
  return entry.time ? `${fmtTime12(entry.time)} | ${entry.name}` : entry.name;
}
...
let typeOrder = JSON.parse(localStorage.getItem("wl2dev_type_order") || "[]");
let typeColors = JSON.parse(localStorage.getItem("wl2dev_type_colors") || "{}");
...
window.addEventListener("message", function (ev) {
  ...
  if (ev.data && ev.data.type === GCAL_OAUTH_MESSAGE_TYPE && ev.data.token) {
...
new MutationObserver((muts) => {
  ...
  for (const m of muts)
    if (m.addedNodes.length) {
      ...
      for (const n of m.addedNodes) if (n.nodeType === 1) enhanceDateInputs(n);
    }
  ...
  segs.forEach((el, i) => {
    ...
    el.addEventListener("keydown", (e) => {
      if (e.key === "Backspace" && el.value === "" && i > 0) {
        ...
        el.value = "";
        sync();
      } else if (e.key === "ArrowLeft" && i > 0 && el.selectionStart === 0) {
        e.preventDefault();
        focusSeg(i - 1);
      } else if (
        e.key === "ArrowRight" &&
...
function setEditSyncMode(id, mode) {
  ...
  if (taskBlock) taskBlock.style.display = mode === "task" ? "block" : "none";
  if (eventBlock)
    eventBlock.style.display = mode === "task" ? "none" : "contents";
  const note = document.getElementById("edit-time-label-note-" + id);
  if (note) note.textContent = mode === "task" ? " (shown in task title)" : "";
...
function addEntry() {
  ...
  if (deadlineEnd && deadline && deadlineEnd < deadline) {
    fieldError("f-deadline-end", "End date can't be before the start date.");
    return;
  }
  ...
  if (document.getElementById("f-repeat")?.checked) {
    ...
    if (!dates) return;
    if (!dates.length) {
      toast("No dates generated — check your end date and repeat settings.");
      return;
    }
...
function deleteEntry(id, confirmed) {
  ...
  if (gcalToken)
    entry.syncMode === "task" ? deleteFromGTask(entry) : deleteFromGCal(entry);
  ...
  render();
  updateTrashBadge();
  toastWithUndo(`Moved "${entry.name}" to Trash.`, () => restoreFromTrash(id));
}
...
async function saveEdit(id) {
  ...
  const modeChanged = newMode !== (e0.syncMode === "task" ? "task" : "event");
  ...
  if (snap.deadlineEnd && snap.deadline && snap.deadlineEnd < snap.deadline) {
    fieldError(
  ...
  if (modeChanged && (e0.gcalEventId || e0.gcalTaskId)) {
    ...
    if (!gcalToken || gcalExpired || liveStatus !== "ok") {
      document.getElementById("mode-switch-warn-modal").classList.add("open");
      return false;
    }
  }
...
function restoreEntry(id) {
  ...
  render();
  toast("Entry restored to active.");
  if (e.syncMode === "task" && gcalToken && e.gcalTaskId) patchGTask(e, true);
}
...
function dueExtend(id) {
  ...
  else if (e.syncMode !== "task" && e.gcalEventId && gcalToken) patchGCal(e);
  save();
  render();
...
function syncPickerToValue(inputId, timeStr) {
  ...
  const mVal = String(Math.round((parseInt(m) || 0) / 5) * 5).padStart(2, "0");
...
document.getElementById("bulk-move-modal")?.addEventListener("click", (ev) => {
...
function toggleRepeatUI() {
  ...
  if (checked) {
    ...
    document.getElementById("repeat-calendar-wrap").style.display = "none";
  }
}
...
function openRepeatPopover() {
  ...
  document.querySelector(".repeat-section")?.classList.add("popover-open");
...
function closeRepeatPopover(silent) {
  ...
  getRepeatCaret().classList.remove("show");
  document.querySelector(".repeat-section")?.classList.remove("popover-open");
...
function updateRepeatSummary(hide) {
  ...
  if (repeatType === "weekly") {
    const days = [...document.querySelectorAll("#repeat-weekly-opts .active")]
...
function setRepeatType(type) {
  ...
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.rtype === type));
...
function renderRepeatCal(startVal, endVal) {
  const generatedDates = new Set(generateRepeatDates(startVal, true) || []);
  const startD = new Date(startVal + "T00:00:00");
  ...
  let html = hdrs.map((h) => `<div class="rp-cal-hdr">${h}</div>`).join("");
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const cls = [
      ...
      .join(" ");
    const click =
      isCustom && !isStart ? `onclick="toggleCustomDate('${dateStr}')"` : "";
    html += `<div class="${cls}" ${click}>${d}</div>`;
  }
  grid.innerHTML = html;

  const totalSelected =
    repeatType === "custom" ? customSelectedDates.size : generatedDates.size;
...
function toggleCustomDate(dateStr) {
  if (customSelectedDates.has(dateStr)) customSelectedDates.delete(dateStr);
...
function generateRepeatDates(startDate, silent = false) {
  ...
  if (repeatType === "daily") {
    const n = parseInt(document.getElementById("f-repeat-days").value) || 1;
...
function populateTypeSelects() {
  ...
  const makeOpts = (selected = "") =>
    ...
    types
      .map(
        (t) => `<option${selected === t ? " selected" : ""}>${esc(t)}</option>`,
      )
      .join("");
...
function render() {
  ...
  if (syncAllEl)
    syncAllEl.style.display = gcalToken && viewMode === "active" ? "" : "none";
  ...
  const filtered =
    selectedFilterTypes.length === 0
      ? sorted(source)
      : sorted(source).filter((e) => selectedFilterTypes.includes(e.type));
...
function entryHTML(e, isArchived = false) {
  const cardColor = typeColors[e.type] || null;
  const status = isArchived ? "" : deadlineStatus(e.deadline, e.deadlineEnd);
  const isEditing = editingId === e.id;
  const types = allTypes();
  const typeOpts = types
    .map((t) => `<option${e.type === t ? " selected" : ""}>${esc(t)}</option>`)
    .join("");
  const selectedStyle =
  ...
  const selectCb =
    selectMode && !isArchived
      ...
      : "";
  const cardClick =
    selectMode && !isArchived ? ` onclick="toggleEntrySelect(${e.id})"` : "";
...
function calTextOn(hex) {
  if (!hex || hex[0] !== "#" || hex.length < 7) return "var(--accent-text)";
...
function renderCalendar() {
  ...
  html += `</div>`;
  const prevYearScroll = list.querySelector(".cal-year-grid")?.scrollTop || 0;
  list.innerHTML = html;
    ...
    html +=
      ...
      days
        .map((day, i) => {
          ...
          const maxChips =
            daySingles.length <= fitAll ? fitAll : Math.max(0, fitAll - 1);
...
function calYearHTML(dated) {
  ...
  dated.forEach((e) => {
    ...
    const endDs =
      e.deadlineEnd && e.deadlineEnd > e.deadline ? e.deadlineEnd : e.deadline;
    ...
    for (; d <= stop; d.setDate(d.getDate() + 1)) {
      ...
      if (!dayType[ds] || idx < dayType[ds].idx) dayType[ds] = { idx, color };
    }
  });
...
function openCalEntry(id) {
  const isArch =
    !entries.some((x) => x.id === id) && archived.some((x) => x.id === id);
  const e =
  ...
  const timeTxt = e.time
    ...
    : "";
  const remarkLong =
    e.remark && (e.remark.length > 140 || e.remark.split("\n").length > 2);
...
document.getElementById("cal-day-modal")?.addEventListener("click", (ev) => {
  if (ev.target.id === "cal-day-modal") closeCalDay();
});
document.getElementById("cal-entry-modal")?.addEventListener("click", (ev) => {
...
function matchesSearch(e, q) {
  ...
  const timeBlob = timeSearchBlob(e.time) + " " + timeSearchBlob(e.timeTo);
...
function exportRow(e) {
  const timeStr = e.time ? (e.timeTo ? `${e.time} - ${e.timeTo}` : e.time) : "";
...
function handleImportFile(file) {
  ...
  if (ext === "json") {
    ...
    reader.onload = () => finishImport(parseImportJSON(reader.result), "JSON");
    reader.onerror = () => toast("Could not read file.");
    reader.readAsText(file);
  } else if (ext === "csv") {
    const reader = new FileReader();
    reader.onload = () => finishImport(parseImportCSV(reader.result), "CSV");
    reader.onerror = () => toast("Could not read file.");
    reader.readAsText(file);
  } else if (ext === "xlsx") {
  ...
  return rows
    ...
    .map((r) => {
      ...
      return {
        ...
        deadline: parseImportDate(idx.deadline > -1 ? r[idx.deadline] : ""),
        deadlineEnd: "",
        time,
        timeTo,
...
async function parseImportXLSX(file) {
  ...
  ws.eachRow((row, rowNum) => {
    ...
    if (!name) return;
    const { time, timeTo } = parseImportTime(idx.time > -1 ? v[idx.time] : "");
...
function finishImport(list, label) {
  ...
  if (added && skipped)
    ...
    toast(`Imported ${added} ${label} ${added === 1 ? "entry" : "entries"}.`);
...
function openGCalModal() {
  ...
  document.getElementById("gcal-connected-section").style.display = connected
    ? "block"
    : "none";
...
function closeModeSwitchWarn() {
  document.getElementById("mode-switch-warn-modal").classList.remove("open");
}
document
...
function gcalAuthorise() {
  ...
  const redirect = encodeURIComponent("https://famelx.github.io/Work-Log/");
  ...
  const popup = window.open(authUrl, "gcal_oauth", "width=520,height=620");
...
function gcalApplyToken(t) {
  ...
  clearGCalExpired();
  document.getElementById("gcal-pill").classList.add("connected");
  document.getElementById("gcal-pill-text").textContent = "Calendar connected";
  closeGCalModal();
  render();
...
function buildGCalEvent(entry) {
  const desc = [entry.type ? `Type: ${entry.type}` : "", entry.remark || ""]
    .filter(Boolean)
    .join("\n");
  ...
  const colorId = typeGCalIds[entry.type] || entry.gcalColorId || undefined;
  let start, end;
  ...
  if (time) {
    ...
    // All-day event: GCal end date is exclusive, so add 1 day. Without
    // this a single-day event is zero-duration and iOS's Calendar
    // widget filters it out entirely.
    const endExclusive = new Date(entry.deadlineEnd || entry.deadline);
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (res.status === 400 && /invalid start time/i.test(msg)) {
        ...
        toast("Calendar update failed — " + (msg || `HTTP ${res.status}`));
    }
  } catch {
    if (!silent) toast("Could not reach Google Calendar.");
  }
...
async function pushToGTask(entry, silent = false) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (!silent)
        toast(
          "Tasks error — " + (err.error?.message || "token may have expired."),
        );
    }
  } catch {
...
let tombstones = JSON.parse(localStorage.getItem("wl2dev_tombstones") || "[]"); // [{id, ts}]
...
function buildSyncPayload() {
  return {
    ...
    settingsUpdatedAt: localStorage.getItem("wl2dev_settings_updated") || "",
    entries,
    archived,
    trash,
    tombstones,
...
async function driveUploadSync(fileId, payload) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => null);
    throw new Error(
      err?.error?.message || "Drive upload failed (HTTP " + res.status + ")",
    );
  }
...
function mergeSync(remote) {
  ...
  if (Array.isArray(remote.customTypes)) {
    ...
    [
      ...new Set([...Object.keys(typeColors), ...Object.keys(remoteColors)]),
    ].forEach((n) => {
      if (remoteColors[n] && (remoteNewer || !typeColors[n]))
        mergedColors[n] = remoteColors[n];
    });
    [
      ...new Set([...Object.keys(typeGCalIds), ...Object.keys(remoteGCalIds)]),
    ].forEach((n) => {
      if (remoteGCalIds[n] != null && (remoteNewer || typeGCalIds[n] == null))
    ...
    const coBase = remoteNewer ? remote.customTypeOrder || [] : customTypeOrder;
    ...
    localStorage.setItem("wl2dev_custom_types", JSON.stringify(customTypes));
    localStorage.setItem("wl2dev_type_colors", JSON.stringify(typeColors));
    localStorage.setItem("wl2dev_type_gcal_ids", JSON.stringify(typeGCalIds));
    localStorage.setItem(
      "wl2dev_custom_type_order",
      JSON.stringify(customTypeOrder),
    );
    localStorage.setItem("wl2dev_type_order", JSON.stringify(typeOrder));
    if (remoteNewer) localStorage.setItem("wl2dev_settings_updated", remoteTs);
    populateTypeSelects();
  }
...
async function performSync(manual = false) {
  ...
  if (!gcalToken || gcalExpired) {
    if (manual)
      toast("Connect Google Calendar first — sync needs a live connection.");
    return false;
  }
  syncInProgress = true;
  ...
  try {
    ...
    if (stats.pulled || stats.removed) {
      ...
      updateTrashBadge();
      if (document.getElementById("trash-overlay")?.classList.contains("open"))
        renderTrash();
    }
    updateSyncStatusUI();
...
function openSettings() {
  ...
  settingsDirty = false;
  pendingTheme = localStorage.getItem("wl2dev_current_theme") || "default";
...
function closeSettings() {
  if (settingsDirty) {
    document.getElementById("settings-unsaved-modal").classList.add("open");
    return;
  }
  closeSettingsForce();
}
function cancelUnsavedPrompt() {
  document.getElementById("settings-unsaved-modal").classList.remove("open");
}
function discardSettingsChanges() {
  ...
  localStorage.setItem("wl2dev_saved_themes", JSON.stringify(savedThemes));
  pendingTheme = null;
  pendingCustomEdited = false;
  loadTheme();
  populateTypeSelects();
...
function handleTypeRowDragOver(e) {
  ...
  if (this !== draggedTypeRow) this.style.borderTop = "2px solid var(--accent)";
  return false;
}
...
function saveSettings() {
  ...
  if (before === null || before !== typeFingerprint()) {
    localStorage.setItem("wl2dev_settings_updated", new Date().toISOString());
    syncDirty = true;
    scheduleSync();
  }
...
const CHANGELOG = [
  {
    ...
    latest: true,
    items: [
      "Fixed: all-day entries synced to Google Calendar were created with no duration, so they never appeared in the iOS Calendar widget (they showed in the Google Calendar app itself) — single-day all-day entries now cover a full day",
    ],
  },
  {
    version: "v1.9.3",
...
function renderChangelog() {
  ...
  document.getElementById("cl-prev-btn").disabled = changelogPage >= total - 1;
...
const AI_TOOLS = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          deadline: {
            type: "string",
            description: "Start date (events) or due date (tasks), YYYY-MM-DD",
          },
          deadlineEnd: {
            type: "string",
  ...
  },
  {
    ...
    function: {
      name: "edit_entry",
      description: "Edit an existing entry, identified by its numeric id.",
      parameters: {
        type: "object",
        properties: {
          id: {
  ...
  },
  {
    ...
    function: {
      name: "restore_entry",
      description: "Restore an archived entry back to active, by numeric id.",
      parameters: {
        type: "object",
        properties: { id: { type: "number" } },
        required: ["id"],
...
function executeAITool(name, args) {
  ...
  try {
    if (name === "add_entry") {
      ...
      render();
      if (gcalToken && entry.deadline) {
        entry.syncMode === "task" ? pushToGTask(entry) : pushToGCal(entry);
      }
    ...
    if (name === "edit_entry") {
      ...
      [
        ...
        if (args[k] !== undefined && args[k] !== null)
          e[k] = k === "time" || k === "timeTo" ? padHM(args[k]) : args[k];
      });
...
async function sendAI() {
  ...
  try {
    ...
    removeMsg(loadId);
    appendMsg("Stopped after too many steps -- please try rephrasing.", "bot");
  } catch (err) {
    removeMsg(loadId);
    appendMsg(
...
function applyTokenSet(tokens) {
  const root = document.documentElement;
  THEME_TOKENS.forEach((t) => {
    if (tokens[t.key]) root.style.setProperty("--" + t.key, tokens[t.key]);
  });
}
...
function onTokenHex(key) {
  const v = normalizeHex(document.getElementById("tk-" + key + "-hex").value);
...
function loadTheme() {
  ...
  document.documentElement.className = cur === "default" ? "" : "theme-" + cur;
}
